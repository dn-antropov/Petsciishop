...
15. **Progress/result bridge + JS fallback reduction** — **PARTIAL**. Standard progress/result bridging exists, ECM pool construction/finalization is parity-clean on the WASM-first path, and MCM triple ranking plus per-cell candidate-pool construction now stay on the WASM-first path. The remaining work is compact ECM/MCM progress/result buffers plus shrinking JS fallback/result reconstruction paths
