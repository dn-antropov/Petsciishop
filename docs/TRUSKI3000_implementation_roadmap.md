...
| **Soft contrast penalty** | ✅ | Replaces hard contrast gate in Standard coarse scoring |

---

## Phase 4 — Output & Measurement ✅ MOSTLY COMPLETE
...
| 4.2 | **Per-cell metadata export** | ❌ Missing | No exported per-cell colors/error/detail for UI |
| 4.3 | **Aspect-ratio-correct preview** | ❌ Missing | Preview is raw 320×200, no 4:3 PAR correction |
...
| 4. Output & Measurement | ✅ ~80% | Full quality metrics suite + cellSSIM + test harness. Missing: per-cell metadata export, 4:3 preview |
...
**Current engine state: ~85% of spec implemented with all major perceptual features active. Quality tuning is ongoing. Remaining work is mode selection (Phase 6), WASM performance (Phase 5), and output polish.**
