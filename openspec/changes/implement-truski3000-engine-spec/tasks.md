...
- [ ] 6.4d Reduce JS fallback paths to explicit unavailable/debug paths and validate milestone canaries before any default-path change
  ...
  - CODEX implementation note (2026-03-11): tightened exact lower-bound pruning in the MCM WASM global triple ranker and per-cell pool scorer so candidates that cannot beat the current best/pool floor are skipped before the expensive fg/luma/hue work. Targeted `skeletor` `current-defaults` WASM preview capture stayed byte-identical to the direct-batch baseline and improved `conversionMs` from `18562.5ms` to `14083.7ms` (`24.1%` faster); guard capture `slayer_multi_color.png` also stayed byte-identical and improved from `21837.0ms` to `13705.2ms` (`37.2%` faster).
  - CODEX next-speed plan (2026-03-11): Standard and ECM now share the clearest remaining exact-preserving speed opportunity in the binary WASM kernel. The next binary optimization slice should stay in WASM and target the shared coarse/background scorer plus shared pool builder rather than solver/finalize plumbing. Priority order:
    1. tighten exact lower-bound pruning in `computeBestByBackgroundForBase(...)` so Standard/ECM skip fg work when fixed nonnegative terms (`csfPenalty`, coverage penalty floor, edge penalty floor) already exceed the current best for that background
    2. tighten exact lower-bound pruning in `computeCandidatePoolsForBase(...)` so Standard/ECM skip fg work when fixed nonnegative terms already exceed the current pool floor
    3. if the bounds above are not enough, precompute candidate-level binary metadata in the kernel (`rowBase`, `nSet`, CSF penalty, coverage extremity, and any fixed bg-independent terms) so the shared Standard/ECM loops stop re-deriving them per fg evaluation
  - CODEX validation rule (2026-03-11): treat this shared binary-kernel work like the MCM pruning slice. Use narrow explicit-`wasm` captures first, keep outputs byte-identical, and prefer one Standard fixture plus one ECM fixture before any broader sweep.
